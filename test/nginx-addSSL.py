...
# HTTP request should work (no redirect with addSSL)
result = attacker.succeed("curl -s http://example.com/")
assert "Hello from example.com" in result, f"Expected greeting via HTTP, got: {result}"

# Request to the legitimate virtualHost should work via HTTPS
result = attacker.succeed("curl -s --cacert /etc/ssl/server.pem https://example.com/")
assert "Hello from example.com" in result, f"Expected greeting via HTTPS, got: {result}"
